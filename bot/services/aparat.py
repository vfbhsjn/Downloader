"""Aparat video downloader service."""
...
_URL_RE = re.compile(r"^(?:https?://)?(?:www\.)?aparat\.com/", re.IGNORECASE)


class AparatDownloader(BaseDownloader):
    """Downloads videos from Aparat."""
    ...
    async def get_info(self, url: str, lang: str = "fa") -> dict:
        try:
            ...
            logger.error("Aparat get_info failed for %s: %s", url, err)
            raise DownloadError(get_text("DOWNLOAD_FAILED", lang).format(error=err)) from err
        return {
            "platform": self.platform,
            "title": info.get("title") or "Aparat",
    ...
    async def download(self, url: str, quality: str = "720p", lang: str = "fa") -> dict:
        ...
        try:
            ...
            logger.error("Aparat download failed for %s: %s", url, err)
            raise DownloadError(get_text("DOWNLOAD_FAILED", lang).format(error=err)) from err
