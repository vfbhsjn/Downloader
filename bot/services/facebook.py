...
_URL_RE = re.compile(
    r"^(?:https?://)?(?:(?:www\.)?facebook\.com|fb\.watch|fb\.com)/", re.IGNORECASE
)


class FacebookDownloader(BaseDownloader):
    """Downloads public Facebook videos without login."""
    ...
    async def get_info(self, url: str, lang: str = "fa") -> dict:
        try:
            ...
            logger.error("Facebook get_info failed for %s: %s", url, err)
            raise DownloadError(get_text("DOWNLOAD_FAILED", lang).format(error=err)) from err
        return {
            "platform": self.platform,
            "title": info.get("title") or info.get("description") or "Facebook",
    ...
    async def download(self, url: str, quality: str = "720p", lang: str = "fa") -> dict:
        ...
        try:
            ...
            logger.error("Facebook download failed for %s: %s", url, err)
            raise DownloadError(get_text("DOWNLOAD_FAILED", lang).format(error=err)) from err
